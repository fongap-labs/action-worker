...
Action Worker 使用中央 `GH_CONTROL_TOKEN` 读取源仓事实与 Actions artifact，使用独立 `GH_RELEASE_TOKEN` 写目标分发仓；业务仓不持有目标仓写凭据。源仓和目标仓分别由 `RELEASE_SOURCE_ALLOWLIST`、`RELEASE_TARGET_ALLOWLIST` 控制。
