...
业务仓的 `ACTION_WORKER_TOKEN` 只允许向 Action Worker 发送调度事件；AI Gateway 凭据和跨仓回写凭据不得下沉到业务仓。中央控制域使用 `GITHUB_CONTROL_TOKEN` 读取目标 PR、回写 Review，并向目标 head commit 写入统一 `PR Governance` status。
...
业务仓不得获得目标分发仓写凭据。源读取使用 `GITHUB_CONTROL_TOKEN`；目标发布使用权限收敛到发布目标的 `GITHUB_RELEASE_TOKEN`。
