...
async function main(): Promise<void> {
  ...
  if (gateJobs.length === 0 || !gateJobs.every((item) => typeof item === "string" && item) || new Set(gateJobs).size !== gateJobs.length) {
  ...
  while (Date.now() < deadline) {
    const response = await getGithubJson(`repos/${repository}/actions/workflows/${workflow}/runs?event=pull_request&head_sha=${headSha}&per_page=20`, token);
    const runs = getJsonArray(response, "workflow_runs")
      .filter((item) => isJsonRecord(item) && getJsonString(item, "head_sha") === headSha)
      .sort((left, right) => isJsonRecord(right) && isJsonRecord(left) ? getJsonString(right, "created_at").localeCompare(getJsonString(left, "created_at")) : 0);
    ...
    await appendLines(process.env.GITHUB_STEP_SUMMARY, [
      "### CI Evidence", "", `- workflow: ${evidence.workflow}`, `- run: ${evidence.run_id}`,
      `- conclusion: ${evidence.conclusion}`, `- gate: ${evidence.gate_job}`,
      `- gate conclusion: ${evidence.gate_conclusion}`, "", "Jobs:",
      ...evidence.jobs.map((job) => `- ${job.name}: ${job.conclusion}`),
    ]);
    console.log(output);
    return;
  }
  throw new CliError(`::error::Timed out waiting for repository CI: repository=${repository} head=${headSha} timeout=${timeoutMinutes}m`);
}
